...
class PropertyDetails extends React.Component {
    ...
    componentDidMount()
    {
        ...
        if(givenIDx != '')
        {
            ...
            axios.get('http://localhost:3001/getFullPropertyDetails/'+givenIDx)
                .then(response => {
                    ...
                    if(response.status === 200){
                        try
                        {
                            ...
                            else if(s.code == '+1')
                            {
                                ...
                                let yx = 'data:image/jpg;base64, ' +response.data.error_type[0]["firstImageData"];
                                ...
                                {
                                    this.ownerPhoto = 'data:image/jpg;base64, ' + response.data.error_type[0].ownerDetails[0].ownerPhoto
                                }
    ...
    getNextImage(e){
       ...
       //console.log(this.state);
        ...
        axios.defaults.withCredentials = true;
            axios.post('http://localhost:3001/getSinglePropertyImage', given)
                .then(response => {
                    //console.log("Status Code : ",response.status);
                   // console.log(response);
                    if(response.status === 200){
    ...
    proceedToBook()
    {
        ...
        if(cookie.load('user') && cookie.load('trcker'))
        {
            ...
            {
                console.log(this.state);
                let propertyID = this.state.fullPropertyData.uniqueid;
                let userID =  cookie.load('trcker');
                let startDate = new Date(this.state.fromTo).getTime();
                let endDate = new Date(this.state.fromTo2).getTime();
                let guestCount = window.globalCTX2;
                console.log(propertyID,userID,startDate,endDate);

                let searchData = {propertyID,userID,startDate,endDate,guestCount};
                console.log(searchData);
                //searchData
                axios.defaults.withCredentials = true;
                axios.post('http://localhost:3001/make-booking',searchData)
                    .then(response => {
                        console.log("Status Code : ",response.status);
                        console.log(response);
                        if(response.status === 200){
                        ...
                        <div className="leftDetailsBoxInner col-lg-9">
                            ...
                            <div className="sliderBox"  data-tomove="scroll5" > 
                                ...
                                <div className={this.state.currentImageClass} >
                                    <img src={this.state.currentImagePath} className="sliderDisplayImage" />
                                </div>
                                <div className=
                            ...
                            <div className="col-lg-12">
                                <div className="propertyHeader" data-tomove="scroll1" >
                                    ...
                                    <hr/>
                                    {/**********ABOUT THE OWNER**********/}
                                    <div className="row col-lg-12 uppermargin"  data-tomove="scroll3" onClick={this.viewUser}>
                                        <div className="descHeader col-lg-12">
                                           <br/> Owner Details
                                        </div>
                                        <div className="descDetails col-lg-3">        
                                        <img src={this.ownerPhoto} alt="HomeAway" />                              
                                        </div>
                                        <div className="descDetails bigLeft col-lg-7">          <div className="initOwner">
                                                 Owner/Manager
                                               </div>
