...
import { findUser } from '../../../query/query';
import { updateUserMutation } from '../../../mutation/mutations';
...
class Profile extends Component {
    ...
    componentDidMount()
    {
         /*************GETTING FORM DETAILS FROM THE SERVER***************** */
         const months = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
         let m = this;
         let lsc = this.props.client.query({
            query : findUser,
            variables: {
                rand : sessionStorage.getItem("userID")
            }
        });
        console.log(lsc);
        lsc.then(function(y){
            console.log(y);
            let x = y.data.findUser;
            let jn = x["joinedon"];
            jn = parseInt(jn);
    ...
    updateProfile()
    {
        ...
        else 
        {
            ...
            let tsc = this.props.updateUserMutation({
                variables: {
                    user_random: sessionStorage.getItem("userID"),
                    ...
                    hometown : this.state.userHometown,
                    languages : this.state.userLang,
                    gender : this.state.gender,
                    ex1 : this.state.userLname
...
export default compose(
    withApollo,
    graphql(findUser , { name: 'findUser' }),
