...
class WebsiteHeader extends React.Component {
    ...
    userLogoutWeb()
    {
        ...
        if(m)
        {
            ...
            axios({
                method: 'POST',
                url: 'http://localhost:3001/logoutUser'
                })
    ...
    render()
    {
        ...
        if(cookie.load('user') && cookie.load('trcker') && cookie.load('login-type')=='TRAVELLER')
        {
            loginType  = cookie.load('login-type');
        }
        else if(cookie.load('user') && cookie.load('trcker') && cookie.load('login-type')=='SELLER')
        {
            loginType  = cookie.load('login-type');
        } 
